...
switch ( request:get-parameter("text", "speaker") )
    case "full" return
        ...
        let $filename := replace(tokenize($tei/base-uri(), "/")[last()], '.xml', '.txt')
    ...
    case "speaker" return
        ...
        let $filename := replace(tokenize($tei/base-uri(), "/")[last()], '.xml', '-NAME.txt')
            ...
            ( 
                let $entries as item()+ := 
                    ( 
                        ...
                        let $text := $texts($key)
                        return
                            <entry name="{$name}" type="text/plain" method="store">{$text}</entry>
                    )
                    return
(:                        $entries:)
